...
public class ASTBuilder extends cppBaseVisitor<ASTNode> {
    ...
    public Program visitStart(cppParser.StartContext ctx) {
        ...
        if (ctx.main() != null) {
            main = (MainFunction) visit(ctx.main());
        }
    ...
    public ASTNode visitStmt(cppParser.StmtContext ctx) {
        ...
        } else if (ctx.print_KW() != null) {
            return visit(ctx.print_KW());
        }
        return null;
    }
    ...
    public FunctionDecl visitFuncdecl(cppParser.FuncdeclContext ctx) {
        ...
        return new FunctionDecl(isVirtual, returnType, name, parameters, body,
    ...
    @Override
    public PrintStmt visitPrint_bool(cppParser.Print_boolContext ctx) {
        return new PrintStmt(PrintStmt.PrintType.BOOL,
            getLine(ctx), getColumn(ctx));
    }

    @Override
    public PrintStmt visitPrint_String(cppParser.Print_StringContext ctx) {
        return new PrintStmt(PrintStmt.PrintType.STRING,
            getLine(ctx), getColumn(ctx));
    }

    @Override
    public PrintStmt visitPrint_char(cppParser.Print_charContext ctx) {
        return new PrintStmt(PrintStmt.PrintType.CHAR,
            getLine(ctx), getColumn(ctx));
    }

    @Override
    public PrintStmt visitPrint_int(cppParser.Print_intContext ctx) {
        return new PrintStmt(PrintStmt.PrintType.INT,
            getLine(ctx), getColumn(ctx));
    }
    ...
    public ClassDecl visitClass(cppParser.ClassContext ctx) {
        ...
        return new ClassDecl(name, baseClass, members,
            getLine(ctx), getColumn(ctx));
    }
    ...
    public ConstructorDecl visitKonstruktordecl(cppParser.KonstruktordeclContext ctx) {
        String name = ctx.ID().getText();
        List<ParamDecl> parameters = visitParamdecl(ctx.paradecl());
        Block body = (Block) visit(ctx.block());
    ...
    public List<ParamDecl> visitParamdecl(cppParser.ParadeclContext ctx) {
        ...
        String name = ctx.ID(0).getText();
        parameters.add(new ParamDecl(type, isReference, name,
            getLine(ctx), getColumn(ctx)));
    ...
    public Expression visitEqualityExpr(cppParser.EqualityExprContext ctx) {
        ...
        for (int i = 1; i < ctx.relationalExpr().size(); i++) {
            ...
            BinaryExpr.Operator operator = op.equals("==") ?
                BinaryExpr.Operator.EQ : BinaryExpr.Operator.NE;
    ...
    public Expression visitRelationalExpr(cppParser.RelationalExprContext ctx) {
        ...
        for (int i = 1; i < ctx.additiveExpr().size(); i++) {
            ...
            switch (op) {
                ...
                case "<=": operator = BinaryExpr.Operator.LE; break;
                case ">": operator = BinaryExpr.Operator.GT; break;
                case ">=": operator = BinaryExpr.Operator.GE; break;
    ...
    public Expression visitAdditiveExpr(cppParser.AdditiveExprContext ctx) {
        ...
        for (int i = 1; i < ctx.mulExpr().size(); i++) {
            String op = ctx.getChild(2 * i - 1).getText();
            BinaryExpr.Operator operator = op.equals("+") ?
                BinaryExpr.Operator.ADD : BinaryExpr.Operator.SUB;
    ...
    public Expression visitMulExpr(cppParser.MulExprContext ctx) {
        ...
        for (int i = 1; i < ctx.unaryExpr().size(); i++) {
            ...
            switch (op) {
                case "*": operator = BinaryExpr.Operator.MUL; break;
    ...
    public Expression visitPrimaryExpr(cppParser.PrimaryExprContext ctx) {
        if (ctx.ID() != null && ctx.getChildCount() == 1) {
            ...
            if (charStr.startsWith("\\")) {
                value = processCharEscape(charStr);
            } else {
        ...
        else if (ctx.ID() != null && ctx.ID().size() == 2 && ctx.function() == null) {
            String objectName = ctx.ID(0).getText();
            String memberName = ctx.ID(1).getText();
            return new MemberAccessExpr(objectName, memberName,
                getLine(ctx), getColumn(ctx));
        }
        else if (ctx.ID() != null && ctx.function() != null) {
            String objectName = ctx.ID(0).getText();
            FunctionCall functionCall = (FunctionCall) visit(ctx.function());
            String methodName = functionCall.getFunctionName();
            List<Expression> arguments = functionCall.getArguments();
            return new MethodCallExpr(objectName, methodName, arguments,
                getLine(ctx), getColumn(ctx));
        }
